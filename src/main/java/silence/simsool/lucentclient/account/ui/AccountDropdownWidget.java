...
public class AccountDropdownWidget {
	...
	public static void render(float titleUiScale) {
		...
		SkijaRenderer.text(currentName, WIDGET_X + 42.0f, WIDGET_Y + 16.0f, Fonts.PRETENDARD_MEDIUM, 0xFFFFFFFF, 14.0f);
		...
		if (open) {
			...
			for (Account acc : accounts) {
				...
				if (itemHover) {
					SkijaRenderer.rect(WIDGET_X + 6.0f, curY, WIDGET_WIDTH - 12.0f, ITEM_HEIGHT, 0x22FFFFFF, 8.0f);
				} else if (isActive) {
					SkijaRenderer.rect(WIDGET_X + 6.0f, curY, WIDGET_WIDTH - 12.0f, ITEM_HEIGHT, 0x14FFFFFF, 8.0f);
				}
				...
				SkijaRenderer.text(acc.getUsername(), WIDGET_X + 42.0f, curY + 14.0f, Fonts.PRETENDARD, nameColor, 14.0f);
			...
			if (manager.isLoggingIn()) {
				SkijaRenderer.circle(WIDGET_X + 25.0f, curY + ITEM_HEIGHT / 2.0f, 9.0f, 0x335C8FFF);
				String status = manager.getStatusMessage() != null ? manager.getStatusMessage() : "Opening browser...";
				SkijaRenderer.text(status, WIDGET_X + 44.0f, curY + 14.0f, Fonts.PRETENDARD_MEDIUM, 0xFF5C8FFF, 12.0f);
				...
				SkijaRenderer.circle(plusCenterX, plusCenterY, 11.0f, 0x2EFFFFFF);
				SkijaRenderer.line(plusCenterX - 4.0f, plusCenterY, plusCenterX + 4.0f, plusCenterY, 1.6f, 0xFFE0E2EC);
				SkijaRenderer.line(plusCenterX, plusCenterY - 4.0f, plusCenterX, plusCenterY + 4.0f, 1.6f, 0xFFE0E2EC);

				int addTextColor = addHover ? 0xFFFFFFFF : 0xFFCCD0DC;
				SkijaRenderer.text("Add Account", WIDGET_X + 46.0f, curY + 14.0f, Fonts.PRETENDARD_MEDIUM, addTextColor, 14.0f);
			}
		}
	}
	...
	private static void drawChevron(float cx, float cy, boolean up, int color) {
		...
		float h = 3.0f;
		if (up) {
			SkijaRenderer.line(cx - halfW, cy + h / 2.0f, cx, cy - h / 2.0f, 1.8f, color);
			SkijaRenderer.line(cx, cy - h / 2.0f, cx + halfW, cy + h / 2.0f, 1.8f, color);
		} else {
			SkijaRenderer.line(cx - halfW, cy - h / 2.0f, cx, cy + h / 2.0f, 1.8f, color);
			SkijaRenderer.line(cx, cy + h / 2.0f, cx + halfW, cy - h / 2.0f, 1.8f, color);
		}
	...
	private static void drawCheckmark(float cx, float cy, int color) {
		SkijaRenderer.line(cx - 5.0f, cy, cx - 1.5f, cy + 3.5f, 2.0f, color);
		SkijaRenderer.line(cx - 1.5f, cy + 3.5f, cx + 5.0f, cy - 3.5f, 2.0f, color);
	}

	private static void drawXMark(float cx, float cy, int color) {
		float s = 3.5f;
		SkijaRenderer.line(cx - s, cy - s, cx + s, cy + s, 1.6f, color);
		SkijaRenderer.line(cx - s, cy + s, cx + s, cy - s, 1.6f, color);
	}
