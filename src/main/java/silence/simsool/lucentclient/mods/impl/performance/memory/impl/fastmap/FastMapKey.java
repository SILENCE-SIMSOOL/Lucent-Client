...
public abstract class FastMapKey<T extends Comparable<T>> {
	 ...
	 * @return The index in the value matrix corresponding to the input state with only the value of this property replaced by <code>newValue</code>
	 */
	public final int replaceIn(int mapIndex, Comparable<?> newValue) {
		final int newPartialIndex = toPartialMapIndex(newValue);
		if (newPartialIndex < 0) return -1;
		else return replaceIn(mapIndex, newPartialIndex);
	}
	 ...
	 * @return An integer such that the sum over the returned values for all properties is the state corresponding to the arguments
	 */
	@SuppressWarnings("unchecked")
	public final int toPartialMapIndex(Comparable<?> value) {
		final int internalIndex = property.getInternalIndex((T) value);
		if (internalIndex < 0 || internalIndex >= numValues()) return -1;
		else return toPartialMapIndex(internalIndex);
	}
