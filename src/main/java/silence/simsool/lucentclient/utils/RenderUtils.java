...
public class RenderUtils {
	...
	public static void fillShape(final PoseStack poseStack, final VertexConsumer builder, final VoxelShape shape, final double x, final double y, final double z, final int color) {
	...
	private static void addBox(PoseStack.Pose pose, VertexConsumer builder, float x1, float y1, float z1, float x2, float y2, float z2, int color) {
		// bottom
	...
	private static void vertex(VertexConsumer builder, PoseStack.Pose pose, float x, float y, float z, int color) {
		builder.addVertex(pose, x, y, z).setColor(color).setNormal(pose, 0, 1, 0).setLineWidth(1.0f);
	}
